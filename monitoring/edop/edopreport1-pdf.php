...
mysqli_query($plm_edom, "TRUNCATE TABLE edoptemp01");


//-- ambil id dari exox
$qryid=mysqli_fetch_assoc(mysqli_query($plm_edom, "select id as vid from exoxpotensi where kedop='$xkode'")) or die("select id from exox : ".mysqli_error());
$xid=$qryid['vid'];

//echo $xid."<br>";

$xjnspar=2;
$qhitpar=mysqli_fetch_assoc(mysqli_query($plm_edom, "select count(parameter) as vjpar from edopparameter where jenis='$xjnspar' and deleted=0")) or die("select id from exox : ".mysqli_error());
$xjpar=$qhitpar['vjpar'];

//echo $xjpar."<br>";


//--- insert total nilai (sum) berd. parameter dari eedopdata
mysqli_query($plm_edom, "INSERT INTO edoptemp01 (id_parameter, parameter, aspek, jenis, jumlah) 
(select id_parameter, parameter, aspek, jenis, sum(v1) as jumlah from edopdata where id_potensi='$xid' group by id_parameter)") or die("error insert:".mysqli_error());

// scorepar
mysqli_query($plm_edom, "update edoptemp01 set scorepar=jumlah, kode='$xkode'");

// scoreas
mysqli_query($plm_edom, "update edoptemp01 t1 inner join (select id_parameter, aspek, jenis, round(avg(jumlah),2) as vas from edoptemp01 where jenis='$xjnspar' group by aspek) as t2 
on t1.id_parameter=t2.id_parameter set t1.scoreas = t2.vas") or die("error scoreas2:".mysqli_error());

// scoreas
mysqli_query($plm_edom, "update edoptemp01 t11 inner join (select id_parameter, aspek, jenis, round(sum(scoreas)/3,2) as vdos from edoptemp01 where jenis='$xjnspar') as t21 
...
$pdf->AliasNbPages();
$que=mysqli_fetch_assoc(mysqli_query($plm_edom, "select scoredos from edoptemp01 where scoredos > 0")) or die("error que:".mysqli_error());
...
$quepot=mysqli_query($plm_edom, "select ta, per, utsuas, kodedosen, namadosen, idprogstudi, kodemk, namamk from exoxpotensi where kedop='$xkode'") or die("error quepot : ".mysqli_error());
...
// --- data nilai
$que1=mysqli_query($plm_edom, "select id_parameter, parameter, aspek, scorepar, scoreas, scoredos from edoptemp01")  or die("error que1 :".mysqli_error());
...
// --- LineGraph
$gque=mysqli_query($plm_edom, "select distinct aspek, scoreas as nilai from edoptemp01 where scoreas > 0 order by aspek")  or die("error as:".mysqli_error());
...
$pdf->SetFont('Arial','',9);
$quekom=mysqli_query($plm_edom, "select kedop, komentar_d from exoxpotensi where kedop='$xkode' and komentar_d <> ''")  or die("error quekom :".mysqli_error());
$nou=0;
