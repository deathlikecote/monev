...
class MY_Controller extends CI_Controller {
	...
	protected function str_thn_akademik_filter( $where_str = '', $fieldcrte = 'ALL', $fieldname = 'thn_akademik', $or = 0 ){
	...
	protected function ta_opts_from_absen_surat( $where_str = NULL, $all = 0) {
		...
		if ($que->num_rows()) {
			...
			foreach ($que->result_array() as $row) {
				...
				$option_str .= '<option value="'.$row['thn_akademik'].'">TA '.$row['thn_akademik'].'</option>';
			}
		}
	...
	public function thn_akademik_by_tgl($tgl = 'NOW') {
