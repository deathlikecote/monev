...
<!-- end breadcrumb -->
<!-- begin page-header -->
<h1 class="page-header">Executive Summary <small>&nbsp;EDOP</small></h1>
<!-- end page-header -->
<?php
		...
		<div class="panel panel-inverse">
			...
			<div class="panel-body">
                <?php 
				 ...
				 ?>
				<form action="index.php?page=es-edop" name="isian" class="form-inline m-b-10" method="POST" >
                    ...
                    <div class="form-group">
		    				<a type="button" href="es/export-es-edop.php?perta=<?=$pertax?>" class="btn btn-success"><i class="fa fa-print"></i> Export</a>
                ...
                $a=0;
                $No=1;
                $sql=mysqli_query($Open,'select * from edopdata_es '.$wheres.' order by namadosen,kodemk asc');
                while($r=mysqli_fetch_array($sql)){

                if($a==0){
                $kdosen=strtoupper($r['kodedosen']);
                $sql1=mysqli_query($Open,'select SUM(total) as total from edopdata_es where kodedosen= "'.$kdosen.'" and ta ="'.$pertax.'"');
                $tots=mysqli_fetch_array($sql1);

                $sql2=mysqli_query($Open,'select * from edopdata_es where kodedosen= "'.$kdosen.'" and ta ="'.$pertax.'"');
                ...
                $a=1;
                    ...
                    if(strtoupper($r['kodedosen'])!=$kdosen){
                        $No=$No+1;
                        $sql1=mysqli_query($Open,'select SUM(total) as total from edopdata_es where kodedosen= "'.$r['kodedosen'].'" and ta ="'.$pertax.'" ');
                        $tots=mysqli_fetch_array($sql1);
                        
                        $sql2=mysqli_query($Open,'select * from edopdata_es where kodedosen= "'.$r['kodedosen'].'" and ta ="'.$pertax.'"');
                        $pemba=mysqli_num_rows($sql2);
                        $totalasli = $tots['total']/$pemba;
                        echo"
                            <tr>
                ...
                <td style='text-align:center'>".number_format($r['A'],2)."</td>
                <td style='text-align:center'>".number_format($r['B'],2)."</td>
                <td style='text-align:center'>".number_format($r['C'],2)."</td>
                <td style='text-align:center'>".number_format($r['total'],2)."</td>
                </tr>
