...
// --- persiapan epomtemp01 <_ report 2 masuk pake epomtemp01

$xkode = $_POST["kode"];
//$kode="MTB";

mysqli_query($plm_edom, "TRUNCATE TABLE epomtemp01");

//hitung jumlah parameter
$qjpar=mysqli_fetch_assoc(mysqli_query($plm_edom, "select count(id) as jmlpar from epomparameter")) or die("error jmlpar:".mysqli_error());
$vjpar=$qjpar['jmlpar'];

//hitung jumlah siswa yang mengisi quesioner
$qjsis=mysqli_fetch_assoc(mysqli_query($plm_edom, "select count(id) as jmlsis from epomdata where idprogstudi='$xkode'")) or die("error jmlsis:".mysqli_error());
$vjsis=$qjsis['jmlsis']/$vjpar;

mysqli_query($plm_edom, "INSERT INTO epomtemp01 (kode, id_parameter, parameter, aspek, jumlah) 
(select idprogstudi, id_parameter, parameter, aspek, sum(v1) as jumlah from epomdata where idprogstudi='$xkode' group by id_parameter)") or die("error insert:".mysqli_error());

mysqli_query($plm_edom, "UPDATE epomtemp01 SET jmlmhs='$vjsis', scorepar=round(jumlah/'$vjsis',2)") or die("error update:".mysqli_error());
...
mysqli_query($plm_edom, "update epomtemp01 t1 join (select idprogstudi, aspek, sum(v1) as jSC , count(id) as jSis from epomdata group by idprogstudi, aspek) t2 
on t1.kode = t2.idprogstudi and t1.aspek=t2.aspek set t1.jumlah=t2.jSC, t1.jmlmhs=t2.jSis") or die("update : ".mysqli_error());

mysqli_query($plm_edom, "update epomtemp01 set scoreas=round(jumlah/jmlmhs,2)");

///---

mysqli_query($plm_edom, "UPDATE epomtemp01 t111 
...
$qpot=mysqli_fetch_assoc(mysqli_query($plm_edom, "select count(nim) as vpot from epompotensi where idprogstudi='$xkode'"))  or die("error as:".mysqli_error());
$spotsis=$qpot['vpot'];

$que=mysqli_query($plm_edom, "select id, parameter, aspek, scorepar, scoreas, scorekls, kode, jmlmhs from epomtemp01 where kode='$xkode' order by id")  or die("error as:".mysqli_error());
$quetem=mysqli_fetch_array($que);

$que1=mysqli_query($plm_edom, "select count(idprogstudi) as jmlisi from epompotensi where idprogstudi='$xkode' and done=1")  or die("error as:".mysqli_error());
...
$quepot=mysqli_query($plm_edom, "select ta, per, utsuas, idprogstudi from epompotensi where idprogstudi='$xkode'") or die(mysqli_error());
...
// --- data nilai
$que1=mysqli_query($plm_edom, "select id_parameter, parameter, aspek, scorepar, scoreas, kode, jmlmhs from epomtemp01 where kode='$xkode' order by id_parameter")  or die("error as:".mysqli_error());
...
$pdf->SetLeftMargin(30);
// --- LineGraph
$gque=mysqli_query($plm_edom, "select distinct aspek, scoreas as nilai from epomtemp01 where kode='$xkode'")  or die("error as:".mysqli_error());
...
$pdf->SetLeftMargin(30);
$pdf->SetFont('Arial','',9);
$quekom=mysqli_query($plm_edom, "select kode, komentar from epompotensi where idprogstudi='$xkode' and komentar <> ''")  or die("error as:".mysqli_error());
$nou=0;
