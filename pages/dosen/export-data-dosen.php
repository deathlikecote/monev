...
->setTitle('Data Dosen'.$_SESSION['nama_pt'].' '.(date('Y')))
->setSubject('Data Dosen'.$_SESSION['nama_pt'].' '.(date('Y')))
->setDescription('Data Dosen')
->setKeywords('Data Dosen Export')
->setCategory('Data Export');

$spreadsheet->getActiveSheet()->mergeCells('A1:G1');
$spreadsheet->setActiveSheetIndex(0)->setCellValue('A1', (strtoupper($_SESSION['nama_pt'])));
$spreadsheet->setActiveSheetIndex(0)->setCellValue('A2', 'DATA DOSEN | '.(date('Y-m-d')));
...
$i=5; 
$no=1; 
$query = "SELECT * FROM m_dosen ORDER BY kodedosen ASC";
...
while ($row = $res1->fetch_assoc()) {
  $spreadsheet->setActiveSheetIndex(0)
  ->setCellValue('A'.$i, $no)
  ->setCellValue('B'.$i, $row['kodedosen'])
...
// Rename worksheet
$spreadsheet->getActiveSheet()->setTitle('Data-Dosen-'.date('Ymd'));
...
header('Content-Disposition: attachment;filename="Data-Dosen-'.date('Ymd').'.xlsx"');
