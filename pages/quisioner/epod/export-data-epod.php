<div class="row">
  ...
  $spreadsheet->getProperties()->setCreator($_SESSION['nama_pt'])
    ...
    ->setTitle('Data Aspek Parameter EPOD' . $_SESSION['nama_pt'] . ' ' . (date('Y')))
    ->setSubject('Data Aspek Parameter EPOD' . $_SESSION['nama_pt'] . ' ' . (date('Y')))
    ->setDescription('Data Aspek Parameter EPOD')
    ->setKeywords('Data Aspek Parameter EPOD Export')
    ->setCategory('Data Export');

  $spreadsheet->getActiveSheet()->mergeCells('A1:G1');
  $spreadsheet->setActiveSheetIndex(0)->setCellValue('A1', (strtoupper($_SESSION['nama_pt'])));
  $spreadsheet->setActiveSheetIndex(0)->setCellValue('A2', 'DATA ASPEK PARAMETER EPOD | ' . (date('Y-m-d')));
  ...
  $i = 6;
  $no = 1;
  $query = "SELECT * FROM epodparameter ORDER BY aspek, nourut ASC";
  ...
  // Rename worksheet
  $spreadsheet->getActiveSheet()->setTitle('Data-Aspek-Param-EPOD-' . date('Ymd'));
  ...
  header('Content-Disposition: attachment;filename="Data-Aspek-Param-EPOD-' . date('Ymd') . '.xlsx"');
