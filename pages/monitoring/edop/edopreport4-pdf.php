...
mysqli_query($Open, "TRUNCATE TABLE edoptemp03");
mysqli_query($Open, "insert into edoptemp03 (id_parameter, parameter, aspek, jenis) 
(select id, parameter, aspek, jenis from edopparameter$sta where deleted=0)") or die("insert1 edoptemp03 : ".mysqli_error());
mysqli_query($Open, "update edoptemp03 set kode='$xkode'");

// scorepar
//mysqli_query("update edoptemp03 t1 inner join (select id_parameter, kodedosen, aspek, jenis, sum(v1) as vv1 
//from edopdata$sta where jenis='$xjnspar' group by id_parameter) as t2 
//on t1.kode=t2.kodedosen set t1.scorepar = t2.vv1") or die("error scorepar:".mysqli_error());

$qscpar=mysqli_query($Open, "select id_parameter, kodedosen, aspek, jenis, round(avg(v1),2) as vv1 from edopdata$sta where (jenis='$xjnspar' and kodedosen='$xkode') group by id_parameter") or die ("qry scorepar : ".mysqli_error()); 
//echo mysql_num_rows($qscpar)."<br>";
while ($scpar=mysqli_fetch_array($qscpar)) {
	$sidp=$scpar['id_parameter'];
	$sv1=$scpar['vv1'];
//	echo $sidp." - ".$sv1."<br>";
	mysqli_query($Open, "update edoptemp03 set scorepar='$sv1' where id_parameter='$sidp'") or die ("update scorepar : ".mysqli_error()); 
}

$qscpar1=mysqli_query($Open, "select id_parameter, kodedosen, aspek, jenis, round(avg(v1),2) as vv11 from edopdata$sta where (jenis=3 and kodedosen='$xkode') group by id_parameter") or die ("qry scorepar1 : ".mysqli_error()); 
//echo mysql_num_rows($qscpar)."<br>";
while ($scpar1=mysqli_fetch_array($qscpar1)) {
	$sidp1=$scpar1['id_parameter'];
	$sv11=$scpar1['vv11'];
//	echo $sidp." - ".$sv1."<br>";
	mysqli_query($Open, "update edoptemp03 set scorepar='$sv11' where id_parameter='$sidp1'") or die ("update scorepar : ".mysqli_error()); 
}

$slprod='';
$qdftp=mysqli_query($Open, "select distinct(idprogstudi) from edopdata$sta where kodedosen='$xkode'") or die ("qry qdftp : ".mysqli_error()); 
while ($sdprod=mysqli_fetch_array($qdftp)) {
	$sidp2=$sdprod['idprogstudi'];
	$slprod=$slprod.$sdprod['idprogstudi'].' ; ';
}

mysqli_query($Open, "update edoptemp03 t1 inner join (select id_parameter, aspek, jenis, round(avg(scorepar),2) as vas from edoptemp03 where jenis='$xjnspar' group by aspek) as t2 
on t1.id_parameter=t2.id_parameter set t1.scoreas = t2.vas") or die("error scoreas2:".mysqli_error());

mysqli_query($Open, "update edoptemp03 t11 inner join (select id_parameter, aspek, jenis, round(sum(scoreas)/'$jmlaspek',2) as vcas from edoptemp03 where jenis='$xjnspar') as t21 
...
$pdf->AliasNbPages();
$que=mysqli_fetch_assoc(mysqli_query($Open, "select scoredos from edoptemp03 where scoredos > 0")) or die("error que:".mysqli_error());
...
$quepot=mysqli_query($Open, "select ta, per, utsuas, kodedosen, namadosen, idprogstudi, kodemk, namamk from exoxpotensi$sta where kodedosen='$xkode'") or die("error quepot : ".mysqli_error());
...
// --- data nilai
$que1=mysqli_query($Open, "select id_parameter, parameter, aspek, scorepar, scoreas, scoredos from edoptemp03")  or die("error que1 :".mysqli_error());
...
// --- LineGraph
$gque=mysqli_query($Open, "select distinct aspek, scoreas as nilai from edoptemp03 where scoreas > 0 order by aspek")  or die("error as:".mysqli_error());
...
$pdf->SetFont('Arial','',9);
$quekom=mysqli_query($Open, "select kedop, komentar_d from exoxpotensi$sta where kodedosen='$xkode' and komentar_d <> ''")  or die("error quekom :".mysqli_error());
$nou=0;
...
$filename2="edopr4-".$xkode."-".$singper.".pdf";
