<div class="row">
  ...
  $spreadsheet->getProperties()->setCreator($_SESSION['nama_pt'])
    ...
    ->setTitle('Data Aspek Parameter EDOM' . $_SESSION['nama_pt'] . ' ' . (date('Y')))
    ->setSubject('Data Aspek Parameter EDOM' . $_SESSION['nama_pt'] . ' ' . (date('Y')))
    ->setDescription('Data Aspek Parameter EDOM')
    ->setKeywords('Data Aspek Parameter EDOM Export')
    ->setCategory('Data Export');

  $spreadsheet->getActiveSheet()->mergeCells('A1:G1');
  $spreadsheet->setActiveSheetIndex(0)->setCellValue('A1', (strtoupper($_SESSION['nama_pt'])));
  $spreadsheet->setActiveSheetIndex(0)->setCellValue('A2', 'DATA ASPEK PARAMETER EDOM | ' . (date('Y-m-d')));
  ...
  $i = 6;
  $no = 1;
  $query = "SELECT * FROM edomparameter ORDER BY aspek, nourut ASC";
  ...
  // Rename worksheet
  $spreadsheet->getActiveSheet()->setTitle('Data-Aspek-Param-EDOM-' . date('Ymd'));
  ...
  header('Content-Disposition: attachment;filename="Data-Aspek-Param-EDOM-' . date('Ymd') . '.xlsx"');
