...
// --- persiapan epomtemp01

$kode = $_POST["kode"];
mysqli_query($plm_edom, "truncate table epomtemp01");

//hitung jumlah parameter
$qjpar=mysqli_fetch_assoc(mysqli_query($plm_edom, "select count(id) as jmlpar from epomparameter where deleted=0"));
$vjpar=$qjpar['jmlpar'];

//hitung jumlah siswa yang mengisi quesioner
$qjsis=mysqli_fetch_assoc(mysqli_query($plm_edom, "select count(id) as jmlsis from epomdata where kode='$kode'"));
$vjsis=$qjsis['jmlsis']/$vjpar;

mysqli_query($plm_edom, "INSERT INTO epomtemp01 (kode, id_parameter, parameter, aspek, jumlah) 
(select kode, id_parameter, parameter, aspek, sum(v1) as jumlah from epomdata where kode='$kode' group by id_parameter)");

mysqli_query($plm_edom, "UPDATE epomtemp01 SET jmlmhs='$vjsis', scorepar=jumlah/'$vjsis'");
...
mysqli_query($plm_edom, "UPDATE epomtemp01 t11 
...
$que=mysqli_query($plm_edom, "select id, parameter, aspek, scorepar, scoreas, scorekls, kode, jmlmhs from epomtemp01 where kode='$xkode' order by id");
...
$quepot=mysqli_query($plm_edom, "select ta, per, utsuas, idprogstudi, kelas_id from epompotensi where kode='$xkode'");
...
// --- data nilai
$que1=mysqli_query($plm_edom, "select id_parameter, parameter, aspek, scorepar, scoreas, kode, jmlmhs from epomtemp01 where kode='$xkode' order by id_parameter");
...
$pdf->SetLeftMargin(30);
// --- LineGraph
$gque=mysqli_query($plm_edom, "select distinct aspek, scoreas as nilai from epomtemp01 where kode='$xkode'");
...
$pdf->SetLeftMargin(30);
$pdf->SetFont('Arial','',9);
$quekom=mysqli_query($plm_edom, "select kode, komentar from epompotensi where idprogstudi='$pprodi' and kelas_id='$pkls' and komentar <> ''");
$nou=0;
