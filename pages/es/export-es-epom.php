...
->setTitle('EPOM Report'.$_SESSION['nama_pt'].' '.(date('Y')))
->setSubject('EPOM Report'.$_SESSION['nama_pt'].' '.(date('Y')))
->setDescription('EPOM Report')
->setKeywords('EPOM Report')
->setCategory('Data');

$spreadsheet->getActiveSheet()->mergeCells('A1:G1');
$spreadsheet->setActiveSheetIndex(0)->setCellValue('A1', (strtoupper($_SESSION['nama_pt'])));
$spreadsheet->setActiveSheetIndex(0)->setCellValue('A2', 'REKAPITULASI EPOM | '.(date('Y-m-d')));
...
$i=5; 
$no=1; 
$query = "SELECT * FROM epomdata_es WHERE ta = '".$_GET['perta']."' GROUP BY namaprodi,total ORDER BY namaprodi,total ASC";
...
while ($row = $res1->fetch_assoc()) {
  ...
  ->setCellValue('H'.$i, number_format($row['D'],2))
  ->setCellValue('I'.$i, number_format($row['total'],2))
  ;
  $spreadsheet->getActiveSheet()->getStyle('A'.$i.':I'.$i)
    ->getBorders()->getAllBorders()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
  $i++; $no++;
}
...
// Rename worksheet
$spreadsheet->getActiveSheet()->setTitle('Epom-Report-'.date('Ymd'));
...
header('Content-Disposition: attachment;filename="Epom-Report-'.date('Ymd').'.xlsx"');
