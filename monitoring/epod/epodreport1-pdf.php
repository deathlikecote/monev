...
// --- persiapan epodtemp01 <_ report 2 masuk pake epodtemp01
...
//---hitung jumlah parameter & jumlah Pengajar yang mengisi quesioner
$qjpar=mysqli_fetch_assoc(mysqli_query($plm_edom, "select count(id) as jmlpar from epodparameter")) or die("error jmlpar:".mysqli_error());
$vjpar=$qjpar['jmlpar'];

$qjsis=mysqli_fetch_assoc(mysqli_query($plm_edom, "select count(id) as jmlsis from epoddata where idprogstudi='$xkode'")) or die("error jmlsis:".mysqli_error());
$vjsis=$qjsis['jmlsis']/$vjpar;

//--- insert total nilai (sum) berd. parameter dari epoddata
mysqli_query($plm_edom, "INSERT INTO epodtemp01 (kode, id_parameter, parameter, aspek, jumlah) 
(select idprogstudi, id_parameter, parameter, aspek, sum(v1) as jumlah from epoddata where idprogstudi='$xkode' group by id_parameter)") or die("error insert:".mysqli_error());

//--- update jmlpengajar, dan hitung scorepar
mysqli_query($plm_edom, "UPDATE epodtemp01 SET jmlmhs='$vjsis', scorepar=round(jumlah/'$vjsis',2)") or die("error update:".mysqli_error());
//-ok mysqli_query($plm_edom, "update edomtemp01 tt1 inner join (select kode, aspek, avg(scorepar) as vscoreas from edomtemp01 group by kode, aspek) tt2 on tt1.aspek = tt2.aspek set tt1.scoreas = tt2.vscoreas")  or die("error as:".mysqli_error());

///--- hitung scoreaspek dari data berd. prodi dan aspek 
//echo " update jumlah point & jumlah mhs ...<br>";
mysqli_query($plm_edom, "update epodtemp01 t1 join (select idprogstudi, aspek, sum(v1) as jSC , count(id) as jSis from epoddata group by idprogstudi, aspek) t2 
on t1.kode = t2.idprogstudi and t1.aspek=t2.aspek set t1.jumlah=t2.jSC, scoreas=round(t2.jSC/t2.jSis,2)") or die("update : ".mysqli_error());

///--- hitung score prodi dari temp01
mysqli_query($plm_edom, "update epodtemp01 t111 
INNER JOIN (select aspek, round(avg(scoreas),2) as vsas FROM epodtemp01) t211 SET t111.scorekls=t211.vsas") or die("error scoreas:".mysqli_error());

//--- end of epodtemp01

$title='e-POD Report.01';

//$pdf = new PDF();
$pdf = new PDF_onet();
$pdf->AliasNbPages();

$qpot=mysqli_fetch_assoc(mysqli_query($plm_edom, "select count(idprogstudi) as vpot from exoxpotensi where idprogstudi='$xkode'"))  or die("error qpot:".mysqli_error());
$spotsis=$qpot['vpot'];

$que=mysqli_query($plm_edom, "select id, parameter, aspek, scorepar, scoreas, scorekls, kode, jmlmhs from epodtemp01 where kode='$xkode' order by id")  or die("error que:".mysqli_error());
...
$quepot=mysqli_query($plm_edom, "select ta, per, utsuas, idprogstudi from exoxpotensi where idprogstudi='$xkode'") or die(mysqli_error());
...
$pdf->cell(3,$tgi1,':',0,0,"C");
$pdf->SetFont('Arial','',9);
$pdf->cell(118,$tgi1,$sjmlmhs.' dari '.$spotsis.' Pengajar',0,0,"L");
...
// --- data nilai
$que1=mysqli_query($plm_edom, "select id_parameter, parameter, aspek, scorepar, scoreas, kode, jmlmhs from epodtemp01 where kode='$xkode' order by id_parameter")  or die("error que1:".mysqli_error());
...
//$pdf->SetLeftMargin(30);
// --- LineGraph
$gque=mysqli_query($plm_edom, "select distinct aspek, scoreas as nilai from epodtemp01 where kode='$xkode'")  or die("error gque:".mysqli_error());
...
$pdf->Cell(63,$tgi,'('.number_format($nil1, 2, '.', '').')',0,0,"L");
...
$pdf->SetLeftMargin(30);
$pdf->SetFont('Arial','',9);
$quekom=mysqli_query($plm_edom, "select idprogstudi, komentar_p from exoxpotensi where idprogstudi='$xkode' and komentar_p <>''")  or die("error quekom:".mysqli_error());
$nou=0;
...
//$filename="test1.pdf";
$filename="pdfoutput/epodr1-".$skodedsn."-".$singper.".pdf";
$filename2="epodr1-".$skodedsn."-".$singper.".pdf";
